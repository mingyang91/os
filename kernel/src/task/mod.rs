...
lazy_static! {
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [
            const {
                TaskControlBlock { task_ctx_ptr: 0, task_status: TaskStatus::UnInit }
            };
            MAX_APP_NUM
        ];
        ...
        for i in 0..num_app {
            ...
            // println!("ptr: {:x}", tasks[i].task_ctx_ptr);
...
impl TaskManager {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            core::mem::drop(inner);
            unsafe {
                // println!("current: {:x}, next: {:x}", *current_task_ctx_ptr2, *next_task_ctx_ptr2);
                __switch(
